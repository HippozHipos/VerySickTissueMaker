...
namespace vstm {
	...
	Texture& TextureManager::Load(const std::string& name, const std::string& path, bool genMipmap)
	{
		...
		{
			VSTM_TRACE_LOGINFO("Texture with name \"{}\" not found. Attempting to create new texture.\n", name);
		}
		else 
		{
			VSTM_TRACE_LOGINFO("Texture with name \"{}\" already exists. Attempting to update texture.\n", name);
		}
		m_texture_map[name] = std::make_shared<Texture>(path, genMipmap);
		if (m_texture_map[name]->GetRawData() == nullptr)
		{
			VSTM_TRACE_LOGINFO("Failed to create texture with name \"{}\"\n", name);
